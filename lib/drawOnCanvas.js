function drawOnCanvas() {
	...
	for (i = 0; i < hands[2].length; i++) {
		if (selectedCard == i) {
			draw_card(hands[2][i].draw_rank, hands[2][i].draw_suit, card_width, card_height, handX[2]+i*40/divisor, handY[3], true, false);
		}
		else {
	...
	var scoreTextX = 655/divisor;
	draw_text(dictionary["scoresheet"], big_text, textColor, scoreTextX, 25/divisor);
	draw_text(dictionary["latest_trick"], big_text, textColor, scoreTextX, 175/divisor);
	draw_text(dictionary["communication"], big_text, textColor, scoreTextX, 325/divisor);
	draw_text(dictionary["title_language"], big_text, textColor, scoreTextX, 518/divisor);
	...
	draw_text(dictionary["goat"], big_text, textColor, 73/divisor, 101/divisor);
	if (player_number == 1) { textColor = "yellow" } else { textColor = "white" }
	draw_text(dictionary["sheep"], big_text, textColor, 231/divisor, 36/divisor);
	if (player_number == 2) { textColor = "yellow" } else { textColor = "white" }
	draw_text(dictionary["you"], big_text, textColor, 231/divisor, 551/divisor);
	textColor = "white";

	draw_dealer_button("img/dealer.png", dealX[dealer], dealY[dealer], 24/divisor, 24/divisor);
	...
	// languages
	draw_box("img/smallbox.png", "  En", small_text, "black", smallBoxX[0], smallBoxY[0], smallBoxWidth, boxHeight);
	draw_box("img/smallbox.png", "  Fr", small_text, "black", smallBoxX[1], smallBoxY[0], smallBoxWidth, boxHeight);
	draw_box("img/smallbox.png", "中(繁)", small_text, "black", smallBoxX[2], smallBoxY[0], smallBoxWidth, boxHeight);
	draw_box("img/smallbox.png", "中(简)", small_text, "black", smallBoxX[3], smallBoxY[0], smallBoxWidth, boxHeight);
}
...
function draw_card(draw_rank, draw_suit, width, height, x, y, side, rotate) {
	var image = new Image();
	if (side == true) {
		image.src = 'img/sam/'+draw_suit+'_'+draw_rank+'.png';
	} else {
