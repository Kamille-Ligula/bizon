function drawOnCanvas() {
	...
	if (gamePhase == 0 && player_number == 2) {
		// first bidding
		draw_text(dictionary["decision"], small_text, textColor, scoreTextX, 347/divisor);
		...
		// second bidding
		draw_text(dictionary["free_to_choose_trump"], small_text, textColor, scoreTextX, 347/divisor);
		draw_box("img/box.png", "♥ "+dictionary["suits_caps"][0], big_text, "black", boxX[0], boxY[0], boxWidth, boxHeight);
		draw_box("img/box.png", "♦ "+dictionary["suits_caps"][1], big_text, "black", boxX[1], boxY[0], boxWidth, boxHeight);
		draw_box("img/box.png", "♣ "+dictionary["suits_caps"][2], big_text, "black", boxX[0], boxY[1], boxWidth, boxHeight);
		draw_box("img/box.png", "♠ "+dictionary["suits_caps"][3], big_text, "black", boxX[1], boxY[1], boxWidth, boxHeight);
		draw_box("img/box.png", dictionary["pass"], big_text, "black", boxX[0], boxY[2], boxWidth, boxHeight);
	} else if (gamePhase == 2 || gamePhase == 3) {
		// actual game
		draw_text(dictionary["trump"]+" "+dictionary["suits_caps"][trump].toLowerCase(), small_text, textColor, scoreTextX, 347/divisor);
		...
		draw_text(dictionary["who_bizon"][bizon], small_text, textColor, scoreTextX, 387/divisor);
	}

	if (gamePhase == 5) {
		if (totalPassCount == 6 && trump == undefined) {
			// everyone passed twice
			draw_text(dictionary["passed_twice"], small_text, "white", scoreTextX, 347/divisor);
		} else {
			// Points et infos du vainqueur de la partie
			draw_text(dictionary["you_have"]+" "+String(gamePoints[2])+" "+dictionary["points"], small_text, "white", scoreTextX, 347/divisor);
			draw_text(dictionary["goat_has"]+" "+String(gamePoints[0])+" "+dictionary["points"], small_text, "white", scoreTextX, 367/divisor);
			draw_text(dictionary["sheep_has"]+" "+String(gamePoints[1])+" "+dictionary["points"], small_text, "white", scoreTextX, 387/divisor);
			switch(victory_status) {
			case true:
				draw_text(dictionary["won_bizon"][bizon], small_text, "white", scoreTextX, 407/divisor);
				break;
			case false:
				draw_text(dictionary["lost_bizon"][bizon], small_text, "white", scoreTextX, 407/divisor);
				break;
			case null:
				draw_text(dictionary["tie"], small_text, "white", scoreTextX, 407/divisor);
			}
		}
		draw_box("img/box.png", dictionary["continue"], big_text, "black", boxX[0], boxY[2], boxWidth, boxHeight);
	}
	if (gamePhase == 6) {
		draw_text(dictionary["game_over"], small_text, "white", scoreTextX, 347/divisor);
		/* RUBY CODE :
		if how_many_winners == 1
			case overall_winner
			when 2
				@winning_message_two = Text.new(@dictionary["most_points"][overall_winner], x: @text_x, y: @text_y+30, size: 18, color: "white")
				@winning_message_three = Text.new(@dictionary["congrats"], x: @text_x, y: @text_y+60, size: 18, color: "white")
			else
				@winning_message_two = Text.new(@dictionary["most_points"][overall_winner], x: @text_x, y: @text_y+30, size: 18, color: "white")
				@winning_message_three = Text.new(@dictionary["lost"], x: @text_x, y: @text_y+60, size: 18, color: "white")
			end
		else
			@winning_message_two = Text.new(@dictionary["tie"], x: @text_x, y: @text_y+30, size: 18, color: "white")
			if table_points_min == 2
				@winning_message_three = Text.new(@dictionary["lost"], x: @text_x, y: @text_y+60, size: 18, color: "white")
			end
		end
		*/
