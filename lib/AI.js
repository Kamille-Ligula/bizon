...
function follow_suit_just_higher() {
	...
	if (table_count == 1 && table[firstToPlay].suit == table[limAdd(firstToPlay, 1)].suit) {
		if (table[firstToPlay].rank > table[limAdd(firstToPlay, 1)].rank) {
			play_higher_than = firstToPlay;
		}
		if (table[firstToPlay].rank < table[limAdd(firstToPlay, 1)].rank) {
			play_higher_than = limAdd(firstToPlay, 1);
		}
	} else {
		play_higher_than = firstToPlay;
	}
			...
			if (Number.isNaN(table[i])) {
				...
				if (table[i].suit == trump) {
					if (you_have_to_play_a_higher_trump_than_this != undefined) {
						if (table[i].rank > you_have_to_play_a_higher_trump_than_this) {
	...
	if (table_count == 3) {
		...
		if (table_count == 2) {
			...
			if (player_number == bizon) {
				...
				//alert("bizon_winning_status: "+bizon_winning_status)
				if (must_follow_suit == true) {
					...
					if (bizon_winning_status == true) {
						...
						if (played_card == undefined && limSub(player_number, 1) != bizon 
							&& table[firstToPlay].suit != table[limAdd(firstToPlay, 1)].suit 
							&& table[limAdd(firstToPlay, 1)].suit == trump) {
							follow_suit_just_higher();
						}
					}
					if (played_card == undefined) {play_low()}
