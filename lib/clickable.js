...
*/
			for (i=1; i-1<hand_minus_one; i++) {
				max_offset.push(offset * i);
			};

			for (j=0; j<9-i; j++) {
				max_offset.push(offset * hand_minus_one + 77);
			};

			for (u=0; u<8; u++) {
				if (mouseX > handX[2] + max_offset[u] && mouseX <= handX[2] + max_offset[u+1] && i >= u+1) {
					if (Number.isNaN(selectedCard) == true) {
						if (mouseY > handY[2] && mouseY <= handY[2] + card_height) {
							cardValidator(u+1, i, max_offset);
						}
					}
					else {
						if (selectedCard == u) {
