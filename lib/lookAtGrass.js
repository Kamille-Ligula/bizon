...
function lookAtGrass() {
	...
	if (gamePhase == 0) {
		...
		i = 0;
		while (i < hands[player_number].length) {
			if (hands[player_number][i].rank == 14 && hands[player_number][i].suit != grass.suit) { memo_suit = hands[player_number][i].suit; }
			i += 1;
		}
		...
		if (memo_suit != undefined) {
			while (i < hands[player_number].length) {
				if ((hands[player_number][i].rank == 13 || hands[player_number][i].rank == 12) && (hands[player_number][i].suit == memo_suit)) {
					trump = suit;
					takeCard();
				}
				i += 1;
		...
		suit = 0;
		while (suit < 4) {
			if (how_many(grass.suit, hands[player_number]) >= 3 && suit != grass.suit) {
				trump = suit;
				takeCard();
			}
			suit += 1;
